...
<div>
  ...
  <v-form ref="DocsForm" name="DocsForm"  lazy-validation >
...
export default {
    name: "FileEmailDocuments",    components:{
    ...
    methods:{
      MyFormRemoteLookupChanged(val){
        this.$applog('Documents MyFormRemoteLookupChanged:'+val.name+"=="+JSON.stringify(val));
        for(var y=0;y<this.value.screens.length;y++)
        {
          ...
          for(var i=0;i<_screen.formElements.length;i++)
          {
            ...
            if ((elm.componentType=="MyFormRemoteLookup")&&(elm.filterObjectName==val.name))
            {
                ...
                this.$applog("Documents MyFormRemoteLookup this.value: "+JSON.stringify(elm));      
                ...
                this.$applog("Documents query query.query: "+JSON.stringify(query));      
                this.$applog("Documents MyFormRemoteLookupChanged remoteLookup response data X:"+elm.name);
                this.dataProvider.getRemoteData(elm, query).then((response) => {
                  ...
                  this.$applog("Documents MyFormRemoteLookupChanged remoteLookup response data:"+_elm.name);
