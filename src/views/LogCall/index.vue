...
<Layout>
   ...
   <div id="Task" >
          ...
          <v-list-item>
                <v-list-item-content>
                  <v-list-item-title class="title">
                      <v-icon size="50" >{{Entitydata.screenMetadata.entityIcon}}</v-icon>
      ...
      <div name="tabtwo">
        <v-form ref="NewTaskForm" name="NewTaskForm" lazy-validation >
          <LogCallGeneral :dataObject="dataObject" :customDataObject="customDataObject" />
        </v-form> 
      </div>
    </div>
  ...
  <template v-slot:footer>
    ...
    <v-spacer></v-spacer>
    <v-btn tile large outlined class="ma-2" color="accent" @click="logCallClick()" :loading="loading" >
...
import  LogCallGeneral  from '@/components/LogCall/General'
...
export default {
  ...
  _DataFactory: [],
  components:{
    Layout, Navbar, LogCallGeneral,MyFormSelect
  },
  data() {
    return {
      screenloading:true,
      customDataObject:{
        phonedirection:'phoneout',
        formElements:[{
              name:'subject',
              componentType:'MyFormInput',
              value:'',
              caption:this.$i18n.t('task-screen.subject'),
              required:true
            },{
              name:'details',
              componentType:'MyFormTextArea',
              value:'',
              caption:this.$i18n.t('task-screen.details'),
              required:false
            }],
      },
      loading:false,
  ...
  methods: {
        filterChange(){
          ...
          this.dataProvider.selectEntity(this.$route.params.entity,this.$route.params.entityid).then((response) => {
              this.Entitydata=response;
              this.dataProvider.getFormMetadata("communication",this.Entitydata).then((response) => {
        ...
        logCallClick() {
          ...
          this.dataObject.actiondatetime=ourdate.getFullYear()+"-"+ourdate.getMonth()+"-"+ourdate.getDate()+
                                          " "+ourdate.getHours()+":"+ourdate.getMinutes();
  
          let crmdata={}
          crmdata.data = this.customDataObject;
          this.dataProvider.privateParseData(this.dataObject.screenMetadata);
          ...
          let _that=this;
          this.dataProvider.logCall(crmdata).then(() => {
